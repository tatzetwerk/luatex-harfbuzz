...
--KE        local name = string.gsub(required,"%.","/") .. libsuffix
        local full = string.gsub(required,"%.","/")	--KE
        local path = file.pathpart(full)	--KE
        local name = file.nameonly(full) .. libsuffix	--KE
        local list = kpse.show_path("clua")
        for root in string.gmatch(list,pathsplit) do
            local full = false
            if type(version) == "string" and version ~= "" then
--KE                full = root .. "/" .. version .. "/" .. name
                full = root .. "/" .. path .. "/" .. version .. "/" .. name	--KE
                full = lfs.isfile(full) and full
            end
            if not full then
--KE                full = root .. "/" .. name
                full = root .. "/" .. path .. "/" .. name	--KE
                full = lfs.isfile(full) and full
            end
            if full then
